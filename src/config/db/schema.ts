...
export const user = pgTable(
  'user',
  {
    ...
    image: text('image'),
    projectApp: text('project_app').default('speeddate').notNull(),
  ...
  (table) => [
    ...
    // Filter users by project app
    index('idx_user_project_app').on(table.projectApp),
...
// ========================================
// Datesetmatch - Speed Dating Event Tables
// ========================================
...
export const event = pgTable(
  'speed_event',
  {
    ...
    organizerId: text('organizer_id')
      .notNull()
      .references(() => user.id, { onDelete: 'cascade' }),
    eventNo: text('event_no').unique().notNull(), // 活动编号 EVT-20231204-XXXX
    name: text('name').notNull(),
    description: text('description'),
    ...
    capacity: integer('capacity').notNull(), // 100 or 200
    ...
    // 支付相关
    orderId: text('order_id'), // 关联订单
    price: integer('price').notNull(), // 价格（分）4900 or 9900
    ...
    matchingCompletedAt: timestamp('matching_completed_at'),

    // 邮件发送记录
    postEventEmailSentAt: timestamp('post_event_email_sent_at'),
    matchResultEmailSentAt: timestamp('match_result_email_sent_at'),
  ...
  (table) => [
    // 查询组织者的活动列表
    index('idx_event_organizer_status').on(table.organizerId, table.status),
    // 按活动日期查询
    index('idx_event_date').on(table.eventDate),
    // 按活动编号查询
    index('idx_event_no').on(table.eventNo),
  ]
);
...
export const participant = pgTable(
  'speed_participant',
  {
    ...
    registrationSource: text('registration_source').notNull(), // 'qr_code' | 'link'

    // 选择记录
    hasSubmittedChoices: boolean('has_submitted_choices').default(false).notNull(),
    choicesSubmittedAt: timestamp('choices_submitted_at'),

    // 选择token (用于邮件链接验证)
    choiceToken: text('choice_token'),
    choiceTokenExpiresAt: timestamp('choice_token_expires_at'),
  ...
  (table) => [
    // 查询活动的参与者列表
    index('idx_participant_event_status').on(table.eventId, table.status),
    // 通过邮箱查找参与者
...
/**
 * ParticipantChoice - 参与者选择表
 * 存储参与者对其他人的感兴趣选择
 */
export const participantChoice = pgTable(
  'speed_participant_choice',
  {
    id: text('id').primaryKey(),
    eventId: text('event_id')
      .notNull()
    ...
    // 选择信息
    isInterested: boolean('is_interested').default(true).notNull(),
  ...
  (table) => [
    // 查询参与者的所有选择
    index('idx_choice_event_participant').on(table.eventId, table.participantId),
    // 用于匹配查询：检查双向选择
    index('idx_choice_mutual').on(table.participantId, table.targetParticipantId),
  ]
);
...
export const matchResult = pgTable(
  ...
  (table) => [
    ...
    // 查询参与者的匹配
    index('idx_match_participants').on(table.participant1Id, table.participant2Id),
  ]
);

/**
 * IcebreakerGuide - 破冰指南表
 * 存储破冰问题内容（可在后台编辑）
 */
export const icebreakerGuide = pgTable(
  'speed_icebreaker_guide',
  {
    id: text('id').primaryKey(),
    category: text('category').notNull(), // 'icebreaker' | 'deeper'
  ...
  (table) => [
    // 按分类查询活跃问题
    index('idx_icebreaker_category_active').on(table.category, table.isActive),
  ]
);
